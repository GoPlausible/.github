# Flask server — guide sample

The Flask sample from https://facilitator.goplausible.xyz/guide:
one paid `GET /my-api` route charging **$0.01 in USDC on Algorand TestNet**, verified
and settled by the GoPlausible facilitator (sync client + sync resource server).
Gasless for buyers — no `feePayer` config anywhere; the middleware learns it
from the facilitator's `/supported`.
